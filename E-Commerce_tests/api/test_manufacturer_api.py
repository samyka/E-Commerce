...
from E-Commerce.testing.factories import get_default_shop, create_product, get_random_filer_image
...
def test_manufacturer_api(admin_user):
    ...
    response = client.post("/api/E-Commerce/manufacturer/",
    ...
    response = client.put("/api/E-Commerce/manufacturer/%d/" % manufacturer.id,
                          content_type="application/json",
                          data=json.dumps(manufacturer_data))
    ...
    assert manufacturer.name == manufacturer_data["name"]
    assert manufacturer.url == manufacturer_data["url"]
    response = client.get("/api/E-Commerce/manufacturer/%d/" % manufacturer.id)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert manufacturer.name == data["name"]
    assert manufacturer.url == data["url"]

    response = client.get("/api/E-Commerce/manufacturer/")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert manufacturer.name == data[0]["name"]
    assert manufacturer.url == data[0]["url"]

    response = client.delete("/api/E-Commerce/manufacturer/%d/" % manufacturer.id)
