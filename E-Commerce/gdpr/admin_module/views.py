...
from E-Commerce.core.models import CompanyContact, Contact, PersonContact
from E-Commerce.gdpr.admin_module.forms import (
    GDPRBaseFormPart, GDPRCookieCategoryFormPart
)
from E-Commerce.gdpr.anonymizer import Anonymizer
...
class GDPRView(SaveFormPartsMixin, FormPartsViewMixin, CreateOrUpdateView):
    model = GDPRSettings
    template_name = "E-Commerce/admin/gdpr/edit.jinja"
    base_form_part_classes = [GDPRBaseFormPart, GDPRCookieCategoryFormPart]
    success_url = reverse_lazy("E-Commerce_admin:gdpr.settings")
...
class BaseContactView(SingleObjectMixin, View):
    def get_queryset(self):
        ...
        limited = (settings.E-Commerce_ENABLE_MULTIPLE_SHOPS and settings.E-Commerce_MANAGE_CONTACTS_PER_SHOP and
...
class GDPRDownloadDataView(BaseContactView):
    def post(self, request, *args, **kwargs):
        ...
        from E-Commerce.gdpr.utils import get_all_contact_data
...
class GDPRAnonymizeView(BaseContactView):
    def post(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(reverse("E-Commerce_admin:contact.detail", kwargs=dict(pk=contact.pk)))
