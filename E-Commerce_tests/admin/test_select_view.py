...
from E-Commerce.testing.factories import create_product, get_default_shop, get_shop, create_random_user
...
def test_ajax_select_view_with_products(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "E-Commerce.Product", "some str", admin_user)
    ...
    results = _get_search_results(rf, view, "E-Commerce.Product", None, admin_user)
    assert len(results) == 0

    results = _get_search_results(rf, view, "E-Commerce.Product", "product", admin_user)
    assert len(results) == 1
    assert results[0].get("id") == product.id
    assert results[0].get("name") == product_name_en

    results = _get_search_results(rf, view, "E-Commerce.ShopProduct", "product", admin_user)
    assert len(results) == 1
...
def test_multi_select_with_main_products(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "E-Commerce.Product", "test", admin_user)
    assert len(results) == Product.objects.count()

    results = _get_search_results(rf, view, "E-Commerce.Product", "test", admin_user, "main")
    assert len(results) == 1
...
def test_multi_select_with_sellable_only_products(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "E-Commerce.Product", "test", admin_user, "sellable_mode_only")
...
def test_multi_select_with_product_sales_unit(rf, admin_user):
    ...
    assert len(_get_search_results(rf, view, "E-Commerce.Product", "Product", admin_user, sales_units="g")) == 1
    assert len(_get_search_results(rf, view, "E-Commerce.Product", "Product", admin_user, sales_units="pcs")) == 1
    assert len(_get_search_results(rf, view, "E-Commerce.Product", "Product", admin_user, sales_units="kg")) == 1
    assert len(_get_search_results(rf, view, "E-Commerce.Product", "Product", admin_user, sales_units="oz")) == 1

    assert len(_get_search_results(rf, view, "E-Commerce.Product", "Product", admin_user, sales_units="g,oz")) == 2
    assert len(_get_search_results(rf, view, "E-Commerce.Product", "Product", admin_user, sales_units="g,kg,pcs")) == 3
    assert len(_get_search_results(rf, view, "E-Commerce.Product", "Product", admin_user, sales_units="oz,pcs,g,kg")) == 4
...
def test_ajax_select_view_with_contacts(rf, contact_cls, admin_user):
    ...
    model_name = "E-Commerce.%s" % contact_cls._meta.model_name
...
def test_ajax_select_view_with_categories(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "E-Commerce.Category", "some str", admin_user)
    assert len(results) == 0
    ...
    results = _get_search_results(rf, view, "E-Commerce.Category", category.name, admin_user)
    assert len(results) == 1
...
def test_multiselect_inactive_users_and_contacts(rf, regular_user, admin_user):
    ...
    # contact not in shop
    results = _get_search_results(rf, view, "E-Commerce.PersonContact", "joe", admin_user)
    assert len(results) == 0
...
def test_select_category(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "E-Commerce.Category", "category", admin_user)
    assert len(results) == 2

    # only visible
    results = _get_search_results(rf, view, "E-Commerce.Category", "category", admin_user, search_mode="visible")
    assert len(results) == 1
...
def test_select_supplier(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "E-Commerce.supplier", "supplier", admin_user)
    assert len(results) == 2

    # only enabled
    results = _get_search_results(rf, view, "E-Commerce.supplier", "supplier", admin_user, search_mode="enabled")
    assert len(results) == 1
...
def test_shop_products_with_supplier_filter(rf, admin_user):
    ...
    with override_settings(E-Commerce_ADMIN_SUPPLIER_PROVIDER_SPEC=supplier_provider):
        results = _get_search_results(rf, view, "E-Commerce.ShopProduct", "ok", superuser1)
        assert len(results) == 1
        assert results[0].get("id") == shop_product.id
        assert results[0].get("name") == product_name_en

        results = _get_search_results(rf, view, "E-Commerce.ShopProduct", "ok", superuser2)
        assert len(results) == 0
