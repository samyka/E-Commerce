...
from E-Commerce.core.defaults.order_statuses import create_default_order_statuses
from E-Commerce.core.models import (
    get_person_contact, MutableAddress, Order, OrderLineType, Product
)
from E-Commerce.core.order_creator import OrderCreator
from E-Commerce.core.pricing import get_pricing_module
...
from E-Commerce.testing.mock_population import populate_if_required
from E-Commerce.testing.soup_utils import extract_form_fields
from E-Commerce_tests.admin.test_order_creator import (
    get_frontend_order_state, get_order_from_state
)
from E-Commerce_tests.front.test_checkout_flow import (
    _populate_client_basket, fill_address_inputs
)
from E-Commerce_tests.functional.test_refunds import (
...
from E-Commerce_tests.utils.basketish_order_source import BasketishOrderSource
    ...
    for key, value in extract_form_fields(soup.find('form', id='addresses')).items():
        if not value:
            ...
            if not value and "email" in key:
                value = "test%d@example.E-Commerce.com" % random.random()
            if not value:
                value = "test"
        inputs[key] = value
...
def test_order_received_variables(rf, with_shop_contact):
    ...
    STEP_DATA = [{
        ...
        "actions": [{
            ...
            "recipient": {
                "constant": "someone@E-Commerce.com"
            }
        }]
    }]
    ...
    if with_shop_contact:
        STEP_DATA[0]['actions'].insert(0, {
            ...
            "recipient": {
                "constant": "someoneelse@E-Commerce.com"
            }
        })
