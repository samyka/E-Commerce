...
from E-Commerce.utils import importing
...
class AdminRegexURLPattern(RegexURLPattern):
    ...
    def _get_unauth_response(self, request, reason):
        ...
        login_url = force_str(reverse("E-Commerce_admin:login") + "?" + error_params)
...
def get_edit_and_list_urls(url_prefix, view_template, name_template, permissions=()):
    ...
    :param view_template: A template string for the dotted name of the view class.
                          E.g. "E-Commerce.admin.modules.taxes.views.Tax%sView"
    ...
    if permissions:
        warnings.warn(
            "get_edit_and_list_urls permissions attribute will be deprecated in E-Commerce 2.0 as unused for this util.",
            DeprecationWarning
        )
...
def get_model_url(object, kind="detail", user=None,
    ...
    :type request: E-Commerce.core.models.Shop|None
    ...
    for module in get_modules():
        ...
        try:
            if required_permissions is not None:
                warnings.warn(
                    "required_permissions parameter will be deprecated in E-Commerce 2.0 as unused for this util.",
                    DeprecationWarning
                )
                permissions = required_permissions
            else:
                resolved = resolve(url)
                from E-Commerce.admin.utils.permissions import get_permissions_for_module_url
...
def derive_model_url(model_class, urlname_prefix, object, kind):
    ...
    :param urlname_prefix: URLname prefix. For instance, `E-Commerce_admin:shop_product.`
...
def get_model_front_url(request, object):
    ...
    if not object.pk:
        return None
    if "E-Commerce.front" in settings.INSTALLED_APPS:
        # Best effort to use the default frontend for front URLs.
        try:
            from E-Commerce.front.template_helpers.urls import model_url
