...
- Add custom template tag to render static source urls. The template tag adds
  the E-Commerce version as a suffix to burst caches
...
- Show name for active supplier at all times when multiple suppliers are
  enabled for E-Commerce project (refs to `E-Commerce.admin.supplier_provider:get_supplier`.
...
- Add options JSON field for `E-Commerce.core.models.Contact`.
- Do not allow creating payment when the not paid amount is not valid
- Expose data from `E-Commerce.core.order_creator.SourceLine`
...
- Add new Dockerfile for E-Commerce_workbench
...
- Make text editor use E-Commerce media browser when addin pictures
...
- Make Summernote editor use E-Commerce media browser when adding pictures.
...
- Fixed typo on `E-Commerce_PROVIDES_BLACKLIST` setting name
...
- Add support for Django 1.11 and for now on E-Commerce is installed over Django 1.11
by default. There is still unofficial support for Django 1.8 and 1.9 which
means that after installing E-Commerce you can downgrade your Django and Django
polymorphic versions. We will also still run tests for these old versions
while adding new features to E-Commerce. See tox.ini for information about
downgrades required to run E-Commerce with old Django.
...
- Add ``E-CommerceMiddleware`` to set the current request shop
...
- API: ``E-Commerce.front.apps.auth.forms.PasswordRecoveryForm`` to core
...
- Add E-Commerce_CHECKOUT_CONFIRM_FORM_PROPERTIES setting which can be used
...
- Fix default OrderStatus identifiers and add a management command
  ``E-Commerce_fix_order_status_identifiers`` to fix them in the database too
...
- Add E-Commerce_PERSON_CONTACT_FIELD_PROPERTIES setting which can be used
  to change person contact form field properties
- Fix caching of ``E-Commerce.product.is_visible`` template function
...
- Add E-Commerce version to static urls
...
- ``E-Commerce_REFERENCE_NUMBER_METHOD``, ``E-Commerce_REFERENCE_NUMBER_LENGTH``
  and ``E-Commerce_REFERENCE_NUMBER_PREFIX`` are now mere defaults and can be
  changed from settings under main menu "Settings > Other Settings >
  System Settings".
- Changed ``E-Commerce_REFERENCE_NUMBER_LENGTH`` from 10 to 17
...
- Add the setting ``E-Commerce_ERROR_PAGE_HANDLERS_SPEC`` to handle custom error
...
- Add ``E-Commerce_AUTO_SHOP_PRODUCT_CATEGORIES`` option that
...
- Make form modifiers reusable. Users of ``ShipmentFormModifier``
  ...
  ``E-Commerce.admin.form_modifier.FormModifier`` interface instead
...
- Remove shop languages, category, tax class, service provider and services
  default record creation from ``E-Commerce_init`` management command
...
- Configure category and search sorts and filters.
    ...
    - This change should be noted when updating latest
      front for projects using ``E-Commerce.front``
...
- Use default address form from core for customer information and
   checkout address.
- Move E-Commerce_FRONT_ADDRESS_FIELD_PROPERTIES to core and rename it to
   E-Commerce_ADDRESS_FIELD_PROPERTIES.
...
- Prevent E-Commerce from loading if Parler related settings are missing
...
- Move ``E-Commerce/order_printouts/pdf_export.py`` to ``E-Commerce/utils/pdf.py``
...
The first E-Commerce release.

Content of E-Commerce 0.4.0 is same as :doc:`Shoop 4.0.0 <shoop-changelog>`
with all "shoop" texts replaced with "E-Commerce".
