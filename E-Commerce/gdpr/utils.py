...
from E-Commerce.utils.i18n import format_datetime


def add_consent_to_response_cookie(response, cookie_data):
    response.set_cookie(
        key=settings.E-Commerce_GDPR_CONSENT_COOKIE_NAME,
...
def get_all_contact_data(contact):
    ...
    from E-Commerce.gdpr.serializers import GDPRCompanyContactSerializer, GDPRPersonContactSerializer
...
def ensure_gdpr_privacy_policy(shop, force_update=False):
    ...
    if force_update or not gdpr_document:
        ...
        content = loader.render_to_string(
            template_name="E-Commerce/admin/gdpr/privacy_policy_page.jinja",
            context=context
        )
        created = False
...
def create_initial_required_cookie_category(shop):
    from E-Commerce.gdpr.models import GDPRCookieCategory
    if not GDPRCookieCategory.objects.filter(shop=shop).exists():
        cookie_category = GDPRCookieCategory.objects.create(
            shop=shop,
            always_active=True,
            cookies="sessionid,csrftoken,E-Commerce_gdpr_consent,rvp",
            name=_("Required"),
            how_is_used=_(
...
def should_reconsent_privacy_policy(shop, user):
    from E-Commerce.gdpr.models import GDPRUserConsent
...
def create_user_consent_for_all_documents(shop, user):
    ...
    """
    from E-Commerce.gdpr.models import GDPRUserConsent, GDPRSettings
