...
from E-Commerce.front.apps.auth.forms import EmailAuthenticationForm
from E-Commerce.front.signals import login_allowed
from E-Commerce.testing.factories import get_default_shop
from E-Commerce.testing.utils import apply_request_middleware
from E-Commerce_tests.front.utils import FieldTestProvider, login_allowed_signal
from E-Commerce_tests.utils.fixtures import REGULAR_USER_PASSWORD, regular_user

regular_user = regular_user  # noqa

pytestmark = pytest.mark.skipif("E-Commerce.front.apps.auth" not in settings.INSTALLED_APPS,
                                reason="Need E-Commerce.front.apps.auth in INSTALLED_APPS")
...
def test_login_with_invalid_password(client, regular_user, rf):
    ...
    response = client.post(reverse("E-Commerce:login"), data={
...
def test_recover_password_form_with_invalid_email():
    from E-Commerce.core.utils.forms import RecoverPasswordForm
...
def test_email_auth_form(rf, regular_user):
    ...
    prepare_user(regular_user)
    request = apply_request_middleware(rf.get("/"), shop=shop)
    with override_provides("front_auth_form_field_provider", ["E-Commerce_tests.front.utils.FieldTestProvider"]):
        payload = {}
        form = EmailAuthenticationForm(request=request, data=payload)
        assert not form.is_valid()
