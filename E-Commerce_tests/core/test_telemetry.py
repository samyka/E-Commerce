...
from E-Commerce.admin.modules.system import SystemModule
from E-Commerce.admin.views.dashboard import DashboardView
from E-Commerce.core.models import PersistentCacheEntry
...
def test_optin_optout(rf, admin_user):
    with override_settings(E-Commerce_TELEMETRY_ENABLED=True, DEBUG=True):
...
def test_disable(rf, admin_user):
    with override_settings(E-Commerce_TELEMETRY_ENABLED=False):
        _clear_telemetry_submission()
        _backdate_installation_key()
        set_opt_out(False)
        with pytest.raises(TelemetryNotSent) as ei:
...
def test_graceful_error(admin_user):
    ...
    with override_settings(E-Commerce_TELEMETRY_ENABLED=True):
...
def test_disabling_telemetry_hides_menu_item(rf):
    request = rf.get("/")
    with override_settings(E-Commerce_TELEMETRY_ENABLED=True):
        assert any(me.original_url == "E-Commerce_admin:telemetry" for me in SystemModule().get_menu_entries(request))
    with override_settings(E-Commerce_TELEMETRY_ENABLED=False):
        assert not any(me.original_url == "E-Commerce_admin:telemetry" for me in SystemModule().get_menu_entries(request))
