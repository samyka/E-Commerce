...
from E-Commerce.xtheme._theme import get_middleware_current_theme
from E-Commerce.xtheme.editing import add_edit_resources
from E-Commerce.xtheme.resources import (
...
class XthemeTemplate(Template):
    ...
    def render(self, *args, **kwargs):
        ...
        try:
            ...
            if ctx and ctx.name and ctx.name in settings.E-Commerce_XTHEME_EXCLUDE_TEMPLATES_FROM_RESOUCE_INJECTION:
                return content
...
class XthemeEnvironment(Environment):
    ...
    Enable by adding ``"environment": "E-Commerce.xtheme.engine.XthemeEnvironment"``
    ...
    def get_template(self, name, parent=None, globals=None):
        ...
        :rtype: E-Commerce.xtheme.engine.XthemeTemplate
        """
    ...
    def _get_themed_template_names(self, name):
        ...
        `E-Commerce/front/bar.jinja`, we'll look at `mystery/E-Commerce/front/bar.jinja`, finally at `E-Commerce/front/bar.jinja`.

        Mystery theme also can define default template dir let's say `pony`. In this scenario we're looking up
        `E-Commerce/front/bar.jinja` from `mystery/E-Commerce/front/bar.jinja` then at `pony/E-Commerce/front/bar.jinja` and
        finally at the default `E-Commerce/front/bar.jinja`.
        ...
        :rtype: str|list[str]
        """
        if name.startswith("E-Commerce/admin"):  # Ignore the admin.
            return name
