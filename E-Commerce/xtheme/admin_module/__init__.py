# -*- coding: utf-8 -*-
# This file is part of E-Commerce.
#
# Copyright (c) 2012-2019, Shoop Commerce Ltd. All rights reserved.
#
# This source code is licensed under the OSL-3.0 license found in the
# LICENSE file in the root directory of this source tree.
from __future__ import unicode_literals

from django.core.urlresolvers import reverse
from django.template import engines
from django.utils.translation import ugettext_lazy as _
from django_jinja.backend import Jinja2

from E-Commerce.admin.base import AdminModule, MenuEntry, Notification
from E-Commerce.admin.menu import CONTENT_MENU_CATEGORY
from E-Commerce.admin.utils.urls import (
    admin_url, derive_model_url, get_edit_and_list_urls
)
from E-Commerce.admin.views.home import HelpBlockCategory, SimpleHelpBlock
from E-Commerce.xtheme._theme import get_current_theme
from E-Commerce.xtheme.engine import XthemeEnvironment
from E-Commerce.xtheme.models import Snippet


class XthemeAdminModule(AdminModule):
    """
    Admin module for Xtheme.

    Allows theme activation/deactivation and further configuration.
    """
    name = _("E-Commerce Extensible Theme Engine")
    breadcrumbs_menu_entry = MenuEntry(_("Themes"), "E-Commerce_admin:xtheme.config", category=CONTENT_MENU_CATEGORY)

    def get_urls(self):  # doccov: ignore
        return [
            admin_url(
                "^xtheme/guide/(?P<theme_identifier>.+?)/",
                "E-Commerce.xtheme.admin_module.views.ThemeGuideTemplateView",
                name="xtheme.guide"
            ),
            admin_url(
                "^xtheme/configure/(?P<theme_identifier>.+?)/",
                "E-Commerce.xtheme.admin_module.views.ThemeConfigDetailView",
                name="xtheme.config_detail"
            ),
            admin_url(
                "^xtheme/theme",
                "E-Commerce.xtheme.admin_module.views.ThemeConfigView",
                name="xtheme.config"
            )
        ]

    def get_menu_entries(self, request):  # doccov: ignore
        return [
            MenuEntry(
                text=_("Themes"), icon="fa fa-paint-brush",
                url="E-Commerce_admin:xtheme.config",
                category=CONTENT_MENU_CATEGORY,
                ordering=1
            )
        ]

    def get_help_blocks(self, request, kind):
        theme = get_current_theme(request.shop)
        if kind == "quicklink" and theme:
            yield SimpleHelpBlock(
                text=_("Customize the look and feel of your shop"),
                actions=[{
                    "text": _("Customize theme"),
                    "url": reverse("E-Commerce_admin:xtheme.config_detail", kwargs={"theme_identifier": theme.identifier})
                }],
                priority=200,
                category=HelpBlockCategory.STOREFRONT,
                icon_url="xtheme/theme.png"
            )

    def get_notifications(self, request):
        try:
            engine = engines["jinja2"]
        except KeyError:
            engine = None

        if engine and isinstance(engine, Jinja2):  # The engine is what we expect...
            if isinstance(engine.env, XthemeEnvironment):  # ... and it's capable of loading themes...
                if not get_current_theme(request.shop):  # ... but there's no theme active?!
                    # Panic!
                    yield Notification(
                        text=_("No theme is active. Click here to activate one."),
                        title=_("Theming"),
                        url="E-Commerce_admin:xtheme.config"
                    )


class XthemeSnippetsAdminModule(AdminModule):
    name = _("E-Commerce Extensible Theme Engine Snippets")
    breadcrumbs_menu_entry = MenuEntry(
        _("Snippets"),
        "E-Commerce_admin:xtheme_snippet.list",
        category=CONTENT_MENU_CATEGORY
    )

    def get_urls(self):
        return get_edit_and_list_urls(
            url_prefix="^xtheme/snippet",
            view_template="E-Commerce.xtheme.admin_module.views.Snippet%sView",
            name_template="xtheme_snippet.%s"
        ) + [
            admin_url(
                "^xtheme/snippet/(?P<pk>\d+)/delete/$",
                "E-Commerce.xtheme.admin_module.views.SnippetDeleteView",
                name="xtheme_snippet.delete"
            )
        ]

    def get_menu_entries(self, request):
        return [
            MenuEntry(
                text=_("Theme Custom CSS/JS"),
                icon="fa fa-magic",
                url="E-Commerce_admin:xtheme_snippet.list",
                category=CONTENT_MENU_CATEGORY,
                ordering=2
            )
        ]

    def get_model_url(self, object, kind, shop=None):
        return derive_model_url(Snippet, "E-Commerce_admin:xtheme_snippet", object, kind)
