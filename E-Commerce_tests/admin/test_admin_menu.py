...
def test_menu_reset_view(rf, admin_user):
    arrange_url = reverse('E-Commerce_admin:menu.arrange')
    ...
    reset_url = reverse('E-Commerce_admin:menu.reset')
...
def test_is_menu_category_active(rf, admin_user):
    url = reverse('E-Commerce_admin:shop_product.list')
    menu_request = apply_request_middleware(rf.get(url), user=admin_user)
    ...
    with override_settings(E-Commerce_ALWAYS_ACTIVE_MENU_CATEGORY_IDENTIFIERS=[SETTINGS_MENU_CATEGORY]):
