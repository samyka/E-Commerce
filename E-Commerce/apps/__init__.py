...
Every E-Commerce Application should define an app config class derived from
`E-Commerce.apps.AppConfig`:class:.
...
To define settings for a E-Commerce Application, add a ``settings.py`` file
...
See also source code of :obj:`E-Commerce.core.settings`.
...
Applications in :term:`E-Commerce Base` distribution should use the following
rules for naming their settings.

 1. Each setting should be prefixed with the string `E-Commerce_`
 2. Boolean toggle settings should have a verb in imperative mood as
    part of the name, e.g. `E-Commerce_ALLOW_ANONYMOUS_ORDERS`,
    `E-Commerce_ENABLE_ATTRIBUTES` or `E-Commerce_ENABLE_MULTIPLE_SHOPS`.
 3. Setting that is used to locate a replaceable module should have
    ...
    those), e.g. `E-Commerce_PRICING_MODULE_SPEC`.
 4. Setting names do NOT have to be prefixed with the application name.
    For example, `E-Commerce_BASKET_VIEW_SPEC` which is not prefixed with
    `E-Commerce_FRONT` even though it is from `E-Commerce.front` application.
