...
from E-Commerce.testing.modules.content import data
from E-Commerce.testing.modules.content.forms import (
    BEHAVIOR_ORDER_CONFIRM_KEY, BehaviorWizardForm, CONTENT_FOOTER_KEY,
    ContentWizardForm
)
from E-Commerce.admin.utils import wizard
from E-Commerce.admin.views.wizard import WizardView
from E-Commerce.notify.actions.email import SendEmail
from E-Commerce.notify.models import Script
from E-Commerce.notify.script import StepNext
...
from E-Commerce.xtheme import XTHEME_GLOBAL_VIEW_NAME
...
def test_content_form(settings):
    ...
    settings.INSTALLED_APPS.remove("E-Commerce.simple_cms")
    form = ContentWizardForm(shop=shop)
    ...
    settings.INSTALLED_APPS.remove("E-Commerce.xtheme")
    form = ContentWizardForm(shop=shop)
    assert "about_us" not in form.fields
    assert "privacy_policy" not in form.fields
    assert "terms_conditions" not in form.fields
    ...
    settings.INSTALLED_APPS.append("E-Commerce.simple_cms")
    settings.INSTALLED_APPS.append("E-Commerce.xtheme")
...
def test_content_wizard_pane(rf, admin_user, settings):
    settings.E-Commerce_SETUP_WIZARD_PANE_SPEC = [
        "E-Commerce.testing.modules.content.views.ContentWizardPane"
    ]
    shop = get_default_shop()
    ...
    assert response["Location"] == reverse("E-Commerce_admin:dashboard")
...
def test_content_wizard_pane2(rf, admin_user, settings):
    ...
    settings.INSTALLED_APPS.remove("E-Commerce.simple_cms")
    settings.INSTALLED_APPS.remove("E-Commerce.xtheme")
    settings.INSTALLED_APPS.remove("E-Commerce.notify")
    ...
    response = WizardView.as_view()(request)
    assert response.status_code == 302
    assert response["Location"].startswith(reverse("E-Commerce:login"))
    ...
    settings.INSTALLED_APPS.append("E-Commerce.notify")
    assert len(wizard.load_setup_wizard_panes(shop, request, visible_only=False)) == 1
    request = apply_request_middleware(rf.post("/", data=pane_data), user=admin_user)
    response = WizardView.as_view()(request)
    assert response.status_code == 200
