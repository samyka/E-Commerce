...
from E-Commerce.core.models import Order, PaymentStatus, Product
from E-Commerce.front.signals import checkout_complete
...
def _populate_client_basket(client):
    ...
    for i in range(3): # add three different products
        ...
        basket_path = reverse("E-Commerce:basket")
...
def test_order_flow_with_phases(get_shipping_method, shipping_data, get_payment_method, payment_data):
    ...
    addresses_path = reverse("E-Commerce:checkout", kwargs={"phase": "addresses"})
    methods_path = reverse("E-Commerce:checkout", kwargs={"phase": "methods"})
    shipping_path = reverse("E-Commerce:checkout", kwargs={"phase": "shipping"})
    payment_path = reverse("E-Commerce:checkout", kwargs={"phase": "payment"})
    confirm_path = reverse("E-Commerce:checkout", kwargs={"phase": "confirm"})
    ...
    if isinstance(payment_method.payment_processor, PaymentWithCheckoutPhase):
        ...
        process_payment_path = reverse(
            "E-Commerce:order_process_payment",
            kwargs={"pk": order.pk, "key": order.key})
        process_payment_return_path = reverse(
            "E-Commerce:order_process_payment_return",
            kwargs={"pk": order.pk, "key": order.key})
        order_complete_path = reverse(
            "E-Commerce:order_complete",
            kwargs={"pk": order.pk, "key": order.key})
