...
from E-Commerce.addons.manager import (
...
class AddonEnableDisableForm(forms.Form):
    ...
    def _create_fields(self):
        enabled_addons = get_enabled_addons(settings.E-Commerce_ENABLED_ADDONS_FILE)
...
class AddonListView(FormView):
    template_name = "E-Commerce/admin/addons/list.jinja"
    form_class = AddonEnableDisableForm

    def form_valid(self, form):
        old_enabled_addons = get_enabled_addons(settings.E-Commerce_ENABLED_ADDONS_FILE)
        ...
        if changes:
            set_enabled_addons(
                settings.E-Commerce_ENABLED_ADDONS_FILE,
                new_enabled_addons,
                comment="Written via E-Commerce admin (user %s; IP %s; time %s)" % (
    ...
    def get_context_data(self, **kwargs):
        ...
        context["toolbar"] = Toolbar([
            ...
            NewActionButton(
                reverse("E-Commerce_admin:addon.upload"),
        ...
        if self.request.GET.get("reload"):
            context["toolbar"].append(
                URLActionButton(
                    reverse("E-Commerce_admin:addon.reload"),
