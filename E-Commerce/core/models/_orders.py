...
from E-Commerce.core import taxing
from E-Commerce.core.excs import (
...
from E-Commerce.core.fields import (
...
from E-Commerce.core.signals import (
    order_status_changed, payment_created, refund_created, shipment_created,
    shipment_created_and_processed
)
from E-Commerce.utils.analog import define_log_model, LogEntryKind
from E-Commerce.utils.dates import local_now, to_aware
from E-Commerce.utils.money import Money
from E-Commerce.utils.properties import (
...
class OrderStatus(TranslatableModel):
    identifier = InternalIdentifierField(
        ...
        unique=True, help_text=_("Internal identifier for status. This is used to identify the statuses in E-Commerce."))
...
class OrderStatusManager(object):
    ...
    def ensure_default_statuses(self):
        ...
        will ensure that
        """
        # these values are based on old E-Commerce data
        update_map = {
...
class Order(MoneyPropped, models.Model):
    ...
    def _cache_values(self):
        ...
        if not self.label:
            self.label = settings.E-Commerce_DEFAULT_ORDER_LABEL
    ...
    def save(self, *args, **kwargs):
        if not self.creator_id:
            if not settings.E-Commerce_ALLOW_ANONYMOUS_ORDERS:
                raise ValidationError(
                    "Anonymous (userless) orders are not allowed "
                    "when E-Commerce_ALLOW_ANONYMOUS_ORDERS is not enabled.")
        self._cache_values()
    ...
    def create_payment(self, amount, payment_identifier=None, description=''):
        ...
        :rtype: E-Commerce.core.models.Payment
        """
    ...
    def create_shipment(self, product_quantities, supplier=None, shipment=None):
        ...
        :type product_quantities: dict[E-Commerce.shop.models.Product, decimal.Decimal]
        ...
        :rtype: E-Commerce.core.models.Shipment
        """
    ...
    def create_refund(self, refund_data, created_by=None, supplier=None):
        ...
        for refund in refund_data:
            ...
            if not settings.E-Commerce_ALLOW_ARBITRARY_REFUNDS and (not parent_line or parent_line == "amount"):
            ...
            if parent_line == "amount":
                ...
                if restock_products and quantity and product:
                    from E-Commerce.core.suppliers.enums import StockAdjustmentType
    ...
    def create_shipment_of_all_products(self, supplier=None):
        ...
        :return: Saved, complete Shipment object
        :rtype: E-Commerce.shop.models.Shipment
        """
    ...
    def get_known_additional_data(self):
        ...
        for data_dict, name_mapping in (
                (self.payment_data, settings.E-Commerce_ORDER_KNOWN_PAYMENT_DATA_KEYS),
                (self.shipping_data, settings.E-Commerce_ORDER_KNOWN_SHIPPING_DATA_KEYS),
                (self.extra_data, settings.E-Commerce_ORDER_KNOWN_EXTRA_DATA_KEYS),
        ):
    ...
    def can_edit(self):
        return (
            not settings.E-Commerce_ENABLE_MULTIPLE_SUPPLIERS and
    ...
    def get_available_shipping_methods(self):
        ...
        """
        from E-Commerce.core.models import ShippingMethod
    ...
    def get_available_payment_methods(self):
        ...
        """
        from E-Commerce.core.models import PaymentMethod
