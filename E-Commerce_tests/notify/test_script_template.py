...
from E-Commerce.notify.admin_module.views import (
...
def test_generic_script_template_admin(rf, admin_user, script_template_cls):
    ...
    assert response.url == reverse("E-Commerce_admin:notify.script-template-config", kwargs={"id": identifier})
    ...
    assert response.status_code == 302
    assert response.url == reverse("E-Commerce_admin:notify.script.list")
...
def test_stock_alert_limit_script_template_manual(rf):
    ...
    data = {
        "en-body": "my body",
        "en-subject": "something",
        "base-recipient": "someemail@E-Commerce.com",
        "base-last24hrs": True
    }
    form = script_template.get_form(data=data)
    assert form.is_valid()
    ...
    data.update({
        "en-body": "my body 2",
        "en-subject": "something 2",
        "base-recipient": "someemail@E-Commerce.comzzz",
    })
    form = script_template.get_form(data=data)
    assert form.is_valid()
    edited_script = script_template.update_script(form)
...
def test_dummy_script_template_manual(rf):
    ...
    with override_provides("notify_script_template", [
            "E-Commerce.testing.notify_script_templates:DummyScriptTemplate"
        ]):
        shop = get_default_shop()
        Script.objects.all().delete()
