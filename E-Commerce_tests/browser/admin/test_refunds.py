...
from E-Commerce.utils.i18n import format_money
...
def _check_create_refund_link(browser, order, present):
    url = reverse("E-Commerce_admin:order.create-refund", kwargs={"pk": order.pk})
...
def _test_create_full_refund(browser, live_server, order):
    ...
    url = reverse("E-Commerce_admin:order.create-full-refund", kwargs={"pk": order.pk})
