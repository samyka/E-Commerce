...
from E-Commerce.core.models import OrderLineType, PaymentStatus
...
def test_refunds(admin_user):
    ...
    refund_url = "/api/E-Commerce/order/%s/create_refund/" % order.id
...
def test_refund_entire_order_without_restock(admin_user):
    ...
    client = _get_client(admin_user)
    refund_url = "/api/E-Commerce/order/%s/create_full_refund/" % order.id
...
def test_refund_errors(admin_user):
    ...
    data = {"restock_products": False}
    response = client.post("/api/E-Commerce/order/%s/create_full_refund/" % order.id, data, format="json")
    assert response.status_code == status.HTTP_400_BAD_REQUEST
