...
from E-Commerce.admin.modules.contacts.views import ContactDetailView
from E-Commerce.apps.provides import override_provides
from E-Commerce.core.models import CompanyContact, PersonContact
from E-Commerce.front.apps.customer_information.views import CompanyEditView
from E-Commerce.front.apps.registration.forms import (
...
from E-Commerce_tests.front.utils import (
...
def test_registration(django_user_model, client, requiring_activation):
    ...
    with override_settings(
        E-Commerce_REGISTRATION_REQUIRES_ACTIVATION=requiring_activation,
    ):
...
def test_registration_2(django_user_model, client, requiring_activation):
    ...
    with override_settings(
        ...
        response = client.post(reverse("E-Commerce:registration_register"), data={
            "username": username,
            "email": email,
            "password1": "password",
            "password2": "password",
            "next": reverse('E-Commerce:checkout')
        })
        user = django_user_model.objects.get(username=username)
        assert response.status_code == 302 #redirect
        assert response.url.endswith(reverse('E-Commerce:checkout'))
        assert PersonContact.objects.count() == 1
        contact = PersonContact.objects.first()
        assert contact.in_shop(shop)
        assert contact.in_shop(shop, only_registration=True)  # registered here
...
def test_password_recovery_user_receives_email_1(client):
    ...
    user = get_user_model().objects.create_user(
        ...
        password="asdfg",
        email="random@E-Commerce.local"
    )
    n_outbox_pre = len(mail.outbox)
    client.post(
        reverse("E-Commerce:recover_password"),
        data={
            "email": user.email
        }
    )
...
def test_password_recovery_user_receives_email_3(client):
    ...
    n_outbox_pre = len(mail.outbox)
    # Recover all users with email random@E-Commerce.local
    client.post(
        reverse("E-Commerce:recover_password"),
        data={
            "email": user.email
...
def test_user_will_be_redirected_to_user_account_page_after_activation(client, requiring_activation):
    ...
    with override_settings(
        ...
        if requiring_activation:
            ...
            assert reverse('E-Commerce:customer_edit') == response.request['PATH_INFO'], 'user should be on the account-page.'
        else:
            assert len(mail.outbox) == 0
            assert user.is_active
            assert reverse('E-Commerce:index') == response.request['PATH_INFO']
...
def test_company_registration(django_user_model, client, allow_company_registration,
    ...
    if not allow_company_registration:
        ...
        assert reverse("E-Commerce:registration_register") in response.url
    else:
...
def test_provider_provides_fields(rf, admin_user):
    ...
    with override_provides("front_registration_field_provider", [
        "E-Commerce_tests.front.utils.FieldTestProvider",
    ]):
