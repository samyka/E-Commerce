...
from E-Commerce_tests.utils.fixtures import regular_user, REGULAR_USER_PASSWORD
...
def test_login_with_invalid_password(client, regular_user, rf):
    ...
    response = client.post(reverse("E-Commerce_admin:login"), data={
...
def test_login_with_email_2(client, regular_user, rf):
    ...
    prepare_user(regular_user)
    redirect_target = "/redirect-success/"
    client.post(reverse("E-Commerce_admin:login"), data={
        "username": regular_user.email,
        "password": REGULAR_USER_PASSWORD,
        REDIRECT_FIELD_NAME: redirect_target
    })
    ...
    response = client.post(reverse("E-Commerce_admin:login"), data=payload)
