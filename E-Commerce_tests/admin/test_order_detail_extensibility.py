...
from E-Commerce.admin.modules.orders.utils import OrderInformation
from E-Commerce.admin.modules.orders.views.detail import OrderDetailView
...
def test_order_detail_has_default_toolbar_action_items(rf, admin_user, has_price):
    ...
    create_payment_url = reverse("E-Commerce_admin:order.create-payment", kwargs={"pk": order.pk})
    set_paid_url = reverse("E-Commerce_admin:order.set-paid", kwargs={"pk": order.pk})
...
def test_order_detail_info_row_extend(rf, admin_user):
    ...
    with override_provides("admin_order_information", [
        "E-Commerce_tests.admin.test_order_detail_extensibility:PaymentMethodName",
    ]):
        response = view_func(request, pk=order.pk)
        soup = BeautifulSoup(response.render().content)
