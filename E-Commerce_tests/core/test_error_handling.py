...
from E-Commerce.admin.error_handlers import AdminPageErrorHandler
from E-Commerce.admin.views.dashboard import DashboardView
from E-Commerce.core.error_handling import install_error_handlers
from E-Commerce.front.error_handlers import FrontPageErrorHandler
from E-Commerce.front.views.index import IndexView
from E-Commerce_tests.utils import replace_urls


def setup_function(fn):
    # clear the cache for error handlers
    import E-Commerce.core.error_handling as error_handling
...
def test_front_error_handlers(rf):
    """
    Test that E-Commerce_ERROR_PAGE_HANDLERS_SPEC installs error handlers overwriting custom ones
    """
    with override_settings(
        DEBUG=False,
        E-Commerce_ERROR_PAGE_HANDLERS_SPEC=["E-Commerce.front.error_handlers.FrontPageErrorHandler"],
        MIDDLEWARE_CLASSES=[],
...
def test_admin_error_handlers(rf):
    ...
    with override_settings(
        DEBUG=False,
        E-Commerce_ERROR_PAGE_HANDLERS_SPEC=["E-Commerce.admin.error_handlers.AdminPageErrorHandler"],
        MIDDLEWARE_CLASSES=[],
        TEMPLATES=[  # Overriden to be sure about the contents of our 500.jinja
            {
                "BACKEND": "django_jinja.backend.Jinja2",
...
def test_install_error_handlers(rf):
    ...
    with override_settings(DEBUG=False, E-Commerce_ERROR_PAGE_HANDLERS_SPEC=[], MIDDLEWARE_CLASSES=[]):
