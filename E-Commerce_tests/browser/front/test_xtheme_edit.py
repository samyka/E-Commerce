...
from E-Commerce.testing.browser_utils import page_has_loaded
...
@pytest.mark.skipif(os.environ.get("E-Commerce_TESTS_TRAVIS", "0") == "1", reason="Disable when run through tox.")
...
def test_xtheme_edit_product(admin_user, browser, live_server, settings):
    ...
    first_product_url = "%s%s" % (
        live_server, reverse("E-Commerce:product", kwargs={"pk": first_product.pk, "slug": first_product.slug})
    )
    ...
    second_product_url = "%s%s" % (
        live_server, reverse("E-Commerce:product", kwargs={"pk": second_product.pk, "slug": second_product.slug})
    )
    ...
    third_product_url = "%s%s" % (
        live_server, reverse("E-Commerce:product", kwargs={"pk": third_product.pk, "slug": third_product.slug})
    )
