...
from E-Commerce.simple_cms.models import Page, PageOpenGraphType
...
def test_opengrah_admin(admin_user):
    ...
    response, soup = client.response_and_soup(reverse("E-Commerce_admin:simple_cms.page.new"))
    ...
    response = client.post(reverse("E-Commerce_admin:simple_cms.page.new"), data=payload)
    assert response.status_code == 302
    assert Page.objects.count() == 1
    page = Page.objects.first()

    # check the rendered page in front
    page_url = reverse("E-Commerce:cms_page", kwargs=dict(url=page.url))
    ...
    response = client.post(reverse("E-Commerce_admin:simple_cms.page.edit", kwargs=dict(pk=page.pk)), data=payload)
    assert response.status_code == 302
    assert Page.objects.count() == 1
    page = Page.objects.first()
