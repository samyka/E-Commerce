...
def test_get_by_identifier(admin_user):
    ...
    order.save()
    client = _get_client(admin_user)
    response = client.get("/api/E-Commerce/order/", data={"identifier": order.identifier})
    assert response.status_code == status.HTTP_200_OK
    order_data = json.loads(response.content.decode("utf-8"))
...
def test_get_by_order_date(admin_user):
    ...
    order.save()
    client = _get_client(admin_user)
    response = client.get("/api/E-Commerce/order/", data={"date": today})
    assert response.status_code == status.HTTP_200_OK
    order_data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.get("/api/E-Commerce/order/", data={"date": yesterday})
    assert response.status_code == status.HTTP_200_OK
    order_data = json.loads(response.content.decode("utf-8"))
...
def test_get_by_status(admin_user):
    ...
    order.save()
    client = _get_client(admin_user)
    response = client.get("/api/E-Commerce/order/", data={"status": order.status.id})
    assert response.status_code == status.HTTP_200_OK
    order_data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.get("/api/E-Commerce/order/", data={"status": cancelled_status.id})
    assert response.status_code == status.HTTP_200_OK
    order_data = json.loads(response.content.decode("utf-8"))
    assert len(order_data) == 9
    ...
    response = client.get("/api/E-Commerce/order/", data={"status": old_status.id})
    assert response.status_code == status.HTTP_200_OK
    order_data = json.loads(response.content.decode("utf-8"))
...
def test_complete_order(admin_user):
    ...
    order.save()
    client = _get_client(admin_user)
    response = client.post("/api/E-Commerce/order/%s/complete/" % order.pk)
...
def test_cancel_order(admin_user):
    ...
    order.save()
    client = _get_client(admin_user)
    response = client.post("/api/E-Commerce/order/%s/cancel/" % order.pk)
    assert response.status_code == 200
    response = client.post("/api/E-Commerce/order/%s/cancel/" % order.pk)
    assert response.status_code == 400
