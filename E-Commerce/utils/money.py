...
class Money(numbers.UnittedDecimal):
    ...
    `~E-Commerce.utils.numbers.UnittedDecimal.value` property (preferred way)
    or by casting to `Decimal`.

    Money objects with different currencies cannot be compared or
    calculated with and will raise `~E-Commerce.utils.numbers.UnitMixupError`.
...
def set_precision_provider(precision_provider):
    ...
    `E-Commerce.utils.babel_precision_provider.get_precision`.
