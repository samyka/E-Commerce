...
from E-Commerce.core.utils.name_mixin import NameMixin
...
from ._base import ChangeProtected, E-CommerceModel
...
class Address(NameMixin, E-CommerceModel):
    """
    ...
    @property
    def is_home(self):
        if not settings.E-Commerce_ADDRESS_HOME_COUNTRY:
            return False
        return self.country.code == settings.E-Commerce_ADDRESS_HOME_COUNTRY.upper()
    ...
    def as_string_list(self, locale=None):
        formatter = cached_load("E-Commerce_ADDRESS_FORMATTER_SPEC")
...
class SavedAddress(E-CommerceModel):
    """
