...
from E-Commerce.core.models import Attribute, AttributeType, AttributeVisibility
...
def test_attr_api(admin_user):
    ...
    response = client.post("/api/E-Commerce/attribute/",
    ...
    response = client.put("/api/E-Commerce/attribute/%d/" % attr.id,
                          content_type="application/json",
                          data=json.dumps(attr_data))
    ...
    response = client.get("/api/E-Commerce/attribute/%d/" % attr.id)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get("/api/E-Commerce/attribute/")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.delete("/api/E-Commerce/attribute/%d/" % attr.id)
