...
def get_installation_key():
    """
    Get the unique installation ID for this E-Commerce instance.
...
def is_telemetry_enabled():
    return bool(settings.E-Commerce_TELEMETRY_ENABLED)
...
def get_daily_data(today):
    ...
    for i, day in enumerate(daterange(data_start_date, data_end_date)):
        if i > settings.E-Commerce_MAX_DAYS_IN_TELEMETRY:
            break
...
def get_telemetry_data(request, indent=None):
    ...
    data_dict = {
        ...
        "E-Commerce_version": E-Commerce.__version__,
    }
...
def _send_telemetry(request, max_age_hours, force_send=False):
    ...
    try:
        resp = requests.post(url=settings.E-Commerce_TELEMETRY_URL, data=data, timeout=5)
        if (
            not settings.DEBUG and
            resp.status_code == 200 and
            resp.json().get("support_id") and
            not configuration.get(None, "E-Commerce_support_id")
        ):
