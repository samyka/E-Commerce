...
"""
This module is installed as the `E-Commerce_admin` template function namespace.
"""
...
from E-Commerce.admin import menu
from E-Commerce.admin.breadcrumbs import Breadcrumbs
from E-Commerce.admin.shop_provider import get_shop
from E-Commerce.admin.supplier_provider import get_supplier
from E-Commerce.admin.utils.menu import is_menu_open
from E-Commerce.admin.utils.urls import manipulate_query_string, NoModelUrl
from E-Commerce.apps.provides import get_provide_objects
from E-Commerce.core.models import Shop
from E-Commerce.core.telemetry import is_telemetry_enabled
...
def is_menu_category_active(category, target_url):
    ...
    identifiers = settings.E-Commerce_ALWAYS_ACTIVE_MENU_CATEGORY_IDENTIFIERS
...
def get_front_url(context):
    ...
    if not front_url:
        try:
            front_url = reverse("E-Commerce:index")
...
def get_support_id(context):
    ...
    if is_telemetry_enabled():
        support_id = configuration.get(None, "E-Commerce_support_id")
    return support_id
...
def get_config(context):
    ...
    qs = {"context": url_name}
    return {
        "searchUrl": manipulate_query_string(reverse("E-Commerce_admin:search"), **qs),
        "menuUrl": manipulate_query_string(reverse("E-Commerce_admin:menu"), **qs),
        "browserUrls": get_browser_urls(request),
        "csrf": get_token(request),
        "docsPage": settings.E-Commerce_ADMIN_MERCHANT_DOCS_PAGE,
...
def get_docs_help_url(context, page=""):
    ...
    if page:
        ...
        return urljoin(settings.E-Commerce_ADMIN_MERCHANT_DOCS_PAGE, page)
    return settings.E-Commerce_ADMIN_MERCHANT_DOCS_PAGE
...
def is_multishop_enabled(context):
    return settings.E-Commerce_ENABLE_MULTIPLE_SHOPS is True
...
def is_multiple_suppliers_enabled(context):
    return settings.E-Commerce_ENABLE_MULTIPLE_SUPPLIERS is True
