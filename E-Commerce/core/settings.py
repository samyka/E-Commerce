...
#: The home currency for the E-Commerce installation. All monetary values
#: are implicitly in this currency unless somehow otherwise specified.
E-Commerce_HOME_CURRENCY = "EUR"

#: The home country code (ISO 3166-1 alpha 2) for the E-Commerce installation.
#: Among other things, addresses that would be printed with this country
#: visible are printed with no country.
E-Commerce_ADDRESS_HOME_COUNTRY = None

#: Whether or not anonymous orders (without a ``creator`` user)
#: are allowed.
E-Commerce_ALLOW_ANONYMOUS_ORDERS = True
...
E-Commerce_ORDER_IDENTIFIER_METHOD = "id"
...
#: `E-Commerce.core.pricing.DiscountModule` for details.
E-Commerce_DISCOUNT_MODULES = ["customer_group_discount", "product_discounts"]

#: List of identifiers of order source modifier modules.
#:
#: See `E-Commerce.core.order_creator.OrderSourceModifierModule` for details.
E-Commerce_ORDER_SOURCE_MODIFIER_MODULES = ["basket_campaigns", "discounts_coupon_codes"]
...
#: other order lines.  See :obj:`E-Commerce.core.taxing` for details.
E-Commerce_TAX_MODULE = "default_tax"
...
E-Commerce_ENABLE_ATTRIBUTES = True
...
E-Commerce_ENABLE_MULTIPLE_SHOPS = False
...
#: Indicates whether E-Commerce should restrict Contact access per Shop
#:
...
#: placed an order.
E-Commerce_MANAGE_CONTACTS_PER_SHOP = False
...
E-Commerce_ORDER_LABELS = [
    ("default", "Default"),
]

#: The order label (see ``E-Commerce_ORDER_LABELS``) to apply to orders by default.
#: This should naturally be one of the keys in ``E-Commerce_ORDER_LABELS``.
E-Commerce_DEFAULT_ORDER_LABEL = "default"
...
E-Commerce_ORDER_KNOWN_PAYMENT_DATA_KEYS = []
...
#: in the administration backend.
E-Commerce_ORDER_KNOWN_SHIPPING_DATA_KEYS = []
...
#: in the administration backend.
E-Commerce_ORDER_KNOWN_EXTRA_DATA_KEYS = []

#: A flag to enable/disable the telemetry system
E-Commerce_TELEMETRY_ENABLED = True

#: The host URL for E-Commerce's telemetry (statistics) system
E-Commerce_TELEMETRY_HOST_URL = "https://telemetry.E-Commerce.com"

#: The submission URL for E-Commerce's telemetry (statistics) system
E-Commerce_TELEMETRY_URL = "%s/collect/" % E-Commerce_TELEMETRY_HOST_URL

#: The URL to fetch the E-Commerce installation support id
E-Commerce_SUPPORT_ID_URL = "%s/support-id" % E-Commerce_TELEMETRY_HOST_URL

#: Default cache duration for various caches in seconds
E-Commerce_DEFAULT_CACHE_DURATION = 60 * 30

#: Overrides for default cache durations by key namespace.
#: These override possible defaults in `E-Commerce.core.cache.impl.DEFAULT_CACHE_DURATIONS`.
E-Commerce_CACHE_DURATIONS = {}

#: Whether taxes should be calculated automatically in TaxModule
E-Commerce_CALCULATE_TAXES_AUTOMATICALLY_IF_POSSIBLE = True

#: Spec which defines an address formatter used to
#: format output string of an Address model instances
E-Commerce_ADDRESS_FORMATTER_SPEC = (
    "E-Commerce.core.utils.formatters:DefaultAddressFormatter")

#: Spec which defines an default address model form
E-Commerce_ADDRESS_MODEL_FORM = (
    "E-Commerce.core.utils.forms.MutableAddressForm")
...
E-Commerce_MAX_DAYS_IN_TELEMETRY = 180
...
E-Commerce_AUTO_SHOP_PRODUCT_CATEGORIES = True
...
#:
#: E-Commerce will iterate over all handlers in order to determinate
#: if some can handle the error. In case of no handler can
#: do the job, a blank response will be returned.
#:
#: A handler must be a subclass of `E-Commerce.core.error_handling.ErrorPageHandler`.
#:
#: If no handler is set (blank), E-Commerce will use default Django's handlers.
#:
...
E-Commerce_SHOP_PRODUCT_SUPPLIERS_STRATEGY = (
    "E-Commerce.core.suppliers.FirstSupplierStrategy")

#: Spec which provides the current shop for a given request and set of parameters.
E-Commerce_REQUEST_SHOP_PROVIDER_SPEC = (
    "E-Commerce.core.shop_provider.DefaultShopProvider")
...
#:
E-Commerce_ALLOW_ARBITRARY_REFUNDS = True
