...
from E-Commerce.core.models import Contact, Gender, PersonContact
...
def test_get_by_email(admin_user):
    ...
    response = client.get("/api/E-Commerce/contact/", data={"email": contact.email})
    assert response.status_code == status.HTTP_200_OK
    contact_data = json.loads(response.content.decode("utf-8"))
...
def test_get_by_contact_group(admin_user):
    ...
    response = client.get("/api/E-Commerce/contact/", data={"groups": group.id})
    assert response.status_code == status.HTTP_200_OK
    contact_data = json.loads(response.content.decode("utf-8"))
...
def test_person_contact_api(admin_user):
    ...
    response = client.post("/api/E-Commerce/person_contact/",
    ...
    response = client.put("/api/E-Commerce/person_contact/%d/" % person_contact.id,
                          content_type="application/json",
                          data=json.dumps(contact_data))
    assert response.status_code == status.HTTP_200_OK
    person_contact = PersonContact.objects.first()
    ...
    response = client.get("/api/E-Commerce/person_contact/%d/" % person_contact.id)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.patch("/api/E-Commerce/person_contact/%d/" % person_contact.id,
                            content_type="application/json",
                            data=json.dumps(contact_data))
    assert response.status_code == status.HTTP_200_OK
    person_contact = PersonContact.objects.first()
    ...
    response = client.delete("/api/E-Commerce/person_contact/%d/" % person_contact.id)
