...
from E-Commerce.front.basket import get_basket_order_creator
...
class ConfirmForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        field_properties = settings.E-Commerce_CHECKOUT_CONFIRM_FORM_PROPERTIES
...
class ConfirmPhase(CheckoutPhaseViewMixin, FormView):
    ...
    template_name = "E-Commerce/front/checkout/confirm.jinja"
    ...
    def form_valid(self, form):
        ...
        if order.require_verification:
            response = redirect("E-Commerce:order_requires_verification", pk=order.pk, key=order.key)
        else:
            response = redirect("E-Commerce:order_process_payment", pk=order.pk, key=order.key)
