...
from E-Commerce.core.api.front_orders import sum_order_lines_price
from E-Commerce.core.models import get_person_contact, OrderLine, OrderLineType
...
def test_list_no_orders(admin_user):
    ...
    response = client.get("/api/E-Commerce/front/orders/")
...
def test_retrieve_order(admin_user):
    ...
    response = client.get("/api/E-Commerce/front/orders/{}/".format(order.pk))
    response_data = json.loads(response.content.decode("utf-8"))
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get("/api/E-Commerce/front/orders/{}/".format(100))
    assert response.status_code == 404

    response = client.get("/api/E-Commerce/front/orders/{}/".format(order2.pk))
    assert response.status_code == 404
