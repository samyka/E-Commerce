...
from E-Commerce.core.models import ProductType
...
def test_product_type_api(admin_user):
    ...
    response = client.put("/api/E-Commerce/product_type/%d/" % product_type.id,
                          content_type="application/json",
                          data=json.dumps(product_type_data))
    ...
    response = client.get("/api/E-Commerce/product_type/%d/" % product_type.id)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert product_type.name == data["translations"]["en"]["name"]
    assert set(product_type.attributes.all().values_list("pk", flat=True)) >= set(product_type_data["attributes"])

    response = client.get("/api/E-Commerce/product_type/")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    ...
    # delete
    response = client.delete("/api/E-Commerce/product_type/%d/" % product_type.id)
