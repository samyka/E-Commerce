...
from E-Commerce.admin.toolbar import PostActionButton, Toolbar, URLActionButton
...
from E-Commerce.core.models import Order, OrderLineType
...
class OrderCreateRefundView(UpdateView):
    model = Order
    template_name = "E-Commerce/admin/orders/create_refund.jinja"
    ...
    def _get_line_number_choices(self, supplier):
        ...
        line_number_choices = [("", "---")]
        if settings.E-Commerce_ALLOW_ARBITRARY_REFUNDS and self.object.get_total_unrefunded_amount(supplier).value > 0:
...
class OrderCreateFullRefundView(UpdateView):
    model = Order
    template_name = "E-Commerce/admin/orders/create_full_refund.jinja"
    context_object_name = "order"
    ...
    def get_context_data(self, **kwargs):
        ...
        context["toolbar"] = Toolbar([
            URLActionButton(
                url=reverse("E-Commerce_admin:order.create-refund", kwargs={"pk": self.object.pk}),
                icon="fa fa-check-circle",
                text=_("Cancel"),
