...
The Provides system is E-Commerce's mechanism for discovering and
...
def _get_provide_specs_from_apps(category):
    """
    Load provide spec strings from installed `E-Commerce.apps.AppConfig`s.
    ...
    :rtype: list[str]
    """
    provides_black_list = getattr(settings, "E-Commerce_PROVIDES_BLACKLIST", {})
    if not isinstance(provides_black_list, dict):
        raise ImproperlyConfigured("The setting `E-Commerce_PROVIDES_BLACKLIST` MUST be a dictionary.")

    if category not in _provide_specs:  # (Re)load required?
        provide_list = []
        for app_config in apps.get_app_configs():
            if not isinstance(app_config, AppConfig):
                # No need to look at non-E-Commerce AppConfigen.
                continue
