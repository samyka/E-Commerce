...
def test_cgp_price_api(admin_user):
    ...
    # create
    response = client.post("/api/E-Commerce/cgp_price/",
    ...
    response = client.put("/api/E-Commerce/cgp_price/%d/" % cgp_price.id,
                          content_type="application/json",
                          data=json.dumps(cgp_price_data))
    ...
    # fetch
    response = client.get("/api/E-Commerce/cgp_price/%d/" % cgp_price.id)
    assert response.status_code == status.HTTP_200_OK
    ...
    # list
    response = client.get("/api/E-Commerce/cgp_price/")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    ...
    # delete
    response = client.delete("/api/E-Commerce/cgp_price/%d/" % cgp_price.id)
