...
from E-Commerce.core.models import Shipment
...
def test_get_shipments(admin_user):
    ...
    # list shipments
    response = client.get("/api/E-Commerce/shipment/")
    ...
    # get shipment by id
    response = client.get("/api/E-Commerce/shipment/%s/" % order1.shipments.first().pk)
    assert response.status_code == status.HTTP_200_OK
    shipment_data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.get("/api/E-Commerce/shipment/?product=%s" % product.pk)
    assert response.status_code == status.HTTP_200_OK
    shipment_data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.get("/api/E-Commerce/shipment/?order=%s" % order3.pk)
    assert response.status_code == status.HTTP_200_OK
    shipment_data = json.loads(response.content.decode("utf-8"))
    for ship in shipment_data:
    ...
    response = client.get("/api/E-Commerce/shipment/?shop=%s" % shop1.pk)
    assert response.status_code == status.HTTP_200_OK
    shipment_data = json.loads(response.content.decode("utf-8"))
    for ship in shipment_data:
