...
from E-Commerce.admin.modules.products.views.edit import ProductEditView
from E-Commerce.core import cache
from E-Commerce.core.models import ShippingMode
from E-Commerce.simple_supplier.admin_module.forms import SimpleSupplierForm
from E-Commerce.simple_supplier.admin_module.views import (
    process_alert_limit, process_stock_adjustment, process_stock_managed
)
from E-Commerce.simple_supplier.models import StockAdjustment, StockCount
...
def test_supplier_with_stock_counts_2(rf, admin_user, settings):
    with override_settings(E-Commerce_HOME_CURRENCY="USD", E-Commerce_ENABLE_MULTIPLE_SHOPS=False):
        supplier = get_simple_supplier()
        shop = get_default_shop()
        assert shop.prices_include_tax
        assert shop.currency != settings.E-Commerce_HOME_CURRENCY
        product = create_product("simple-test-product", shop, supplier)
        quantity = random.randint(100, 600)
        supplier.adjust_stock(product.pk, quantity)
        ...
        settings.E-Commerce_ENABLE_MULTIPLE_SHOPS = True
        sa = StockAdjustment.objects.first() # refetch to invalidate cache
        assert sa.purchase_price.currency != shop.currency
        assert sa.purchase_price.currency == settings.E-Commerce_HOME_CURRENCY
        assert not sa.purchase_price.includes_tax
        sc = StockCount.objects.first()
        assert sc.stock_value.currency == settings.E-Commerce_HOME_CURRENCY
        assert not sc.stock_value.includes_tax
        assert sc.stock_unit_price.currency == settings.E-Commerce_HOME_CURRENCY
