...
from E-Commerce_tests.utils.fixtures import REGULAR_USER_PASSWORD, REGULAR_USER_EMAIL
...
def test_reset_admin_user_password_errors(client):
    ...
    # user not active
    response = client.post(reverse("E-Commerce_admin:request_password"), data={"email": user.email})
    assert response.status_code == 302
    assert len(mail.outbox) == 0
    ...
    # non-existent email
    response = client.post(reverse("E-Commerce_admin:request_password"), data={"email": "doesntexist@email.ok"})
    assert response.status_code == 302
    assert len(mail.outbox) == 0
    ...
    response = client.post(
        reverse("E-Commerce_admin:recover_password", kwargs=dict(uidb64=uid, token="invalid")),
    ...
    # invalid uid
    response = client.post(
        reverse("E-Commerce_admin:recover_password", kwargs=dict(uidb64="uid", token=token)),
        data={"new_password1": new_password, "new_password2": new_password}
    )
    assert response.status_code == 400
    assert "This recovery link is invalid" in response.content.decode("utf-8")
