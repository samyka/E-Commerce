...
from E-Commerce.core.models import MutableAddress
from E-Commerce.testing.factories import create_random_person, get_default_shop
...
def test_mutable_address_api(admin_user):
    ...
    response = client.post("/api/E-Commerce/address/",
    ...
    response = client.put("/api/E-Commerce/address/%d/" % mutable_address.id,
                          content_type="application/json",
                          data=json.dumps(mutable_address_data))
    ...
    response = client.get("/api/E-Commerce/address/%d/" % mutable_address.id)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.patch("/api/E-Commerce/address/%d/" % mutable_address.id,
                            content_type="application/json",
                            data=json.dumps(mutable_address_data))
    assert response.status_code == status.HTTP_200_OK
    mutable_address = MutableAddress.objects.first()
    ...
    response = client.delete("/api/E-Commerce/address/%d/" % mutable_address.id)
...
def test_mutable_address_field_properties_settings_api(admin_user, settings):
    settings.E-Commerce_ADDRESS_FIELD_PROPERTIES["postal_code"] = {"required": True}
    settings.E-Commerce_ADDRESS_FIELD_PROPERTIES["street2"] = {"required": True}
    ...
    del(settings.E-Commerce_ADDRESS_FIELD_PROPERTIES["postal_code"])
    del(settings.E-Commerce_ADDRESS_FIELD_PROPERTIES["street2"])
