...
from E-Commerce.admin.dashboard import (
    ChartDataType, ChartType, DashboardChartBlock, DashboardContentBlock,
    DashboardMoneyBlock, MixedChart
)
from E-Commerce.core.models import Order, Shop
from E-Commerce.core.pricing import TaxfulPrice
from E-Commerce.core.utils.query import group_by_period
from E-Commerce.utils.dates import get_year_and_month_format, local_now, to_aware
...
def get_shop_overview_block(request, currency, for_date=None):
    ...
    block = DashboardContentBlock.by_rendering_template(
        "store_overview", request, "E-Commerce/admin/sales_dashboard/_store_overview_dashboard_block.jinja", {
            "daily": daily,
            "mtd": mtd,
            "ytd": ytd,
            "totals": totals
...
def get_recent_orders_block(request, currency=None):
    ...
    block = DashboardContentBlock.by_rendering_template(
        "recent_orders", request, "E-Commerce/admin/sales_dashboard/_recent_orders_dashboard_block.jinja", {
            "orders": orders
        }
    )
    block.size = "medium"
