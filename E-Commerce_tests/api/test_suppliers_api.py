...
from E-Commerce.core.models import Shop, ShopProduct, Supplier
...
from E-Commerce.simple_supplier.models import StockAdjustment
...
def test_get_suppliers(admin_user):
    ...
    # list suppliers
    response = client.get("/api/E-Commerce/supplier/")
    ...
    # get supplier by id
    response = client.get("/api/E-Commerce/supplier/%s/" % supplier2.pk)
    assert response.status_code == status.HTTP_200_OK
    supplier_data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.get("/api/E-Commerce/supplier/%s/stock_statuses/" % supplier1.pk)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get("/api/E-Commerce/supplier/%s/stock_statuses/" % supplier1.pk, format="json", data={
...
def test_adjust_stock(admin_user):
    ...
    # invalid type
    response = client.post("/api/E-Commerce/supplier/%s/adjust_stock/" % supplier1.pk, format="json", data={
    ...
    # invalid supplier
    response = client.post("/api/E-Commerce/supplier/%s/adjust_stock/" % 100, format="json", data={
        "product": sp.product.pk,
        "delta": 100,
    ...
    response = client.post("/api/E-Commerce/supplier/%s/adjust_stock/" % supplier2.pk, format="json", data={
        "product": sp.product.pk,
        "delta": 100,
        "type": StockAdjustmentType.INVENTORY.value
    })
    ...
    # update the stock,
    response = client.post("/api/E-Commerce/supplier/%s/update_stocks/" % supplier1.pk, format="json", data={
