...
from E-Commerce.gdpr.utils import ensure_gdpr_privacy_policy, create_user_consent_for_all_documents, \
...
def test_consent_cookies(rf):
    ...
    for code, lang in settings.LANGUAGES:
        ...
        response = client.get(index_url)
        assert settings.E-Commerce_GDPR_CONSENT_COOKIE_NAME not in response.cookies

        # send consent
        response = client.post(reverse("E-Commerce:gdpr_consent"), data={
        ...
        assert settings.E-Commerce_GDPR_CONSENT_COOKIE_NAME in response.cookies
        cookies_data = json.loads(response.cookies[settings.E-Commerce_GDPR_CONSENT_COOKIE_NAME].value)
        ...
        request.COOKIES = {
            settings.E-Commerce_GDPR_CONSENT_COOKIE_NAME: (
                response.client.cookies[settings.E-Commerce_GDPR_CONSENT_COOKIE_NAME].value
            )
        }
        context = {'request': request}
