...
from E-Commerce.core.models import SalesUnit
...
def test_sales_unit_api(admin_user):
    ...
    response = client.post("/api/E-Commerce/sales_unit/",
    ...
    response = client.put("/api/E-Commerce/sales_unit/%d/" % sales_unit.id,
                          content_type="application/json",
                          data=json.dumps(sales_unit_data))
    ...
    response = client.get("/api/E-Commerce/sales_unit/%d/" % sales_unit.id)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get("/api/E-Commerce/sales_unit/")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.delete("/api/E-Commerce/sales_unit/%d/" % sales_unit.id)
