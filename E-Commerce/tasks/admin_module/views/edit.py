...
from E-Commerce.admin.forms.widgets import QuickAddRelatedObjectSelect
from E-Commerce.admin.shop_provider import get_shop
from E-Commerce.admin.toolbar import get_default_edit_toolbar, PostActionButton
from E-Commerce.admin.utils.views import CreateOrUpdateView
from E-Commerce.core.models import Contact, get_person_contact
from E-Commerce.tasks.models import Task, TaskComment, TaskStatus, TaskType
from E-Commerce.utils.analog import LogEntryKind
from E-Commerce.utils.form_group import FormGroup
...
class QuickAddTaskTypeSelect(QuickAddRelatedObjectSelect):
    url = reverse_lazy("E-Commerce_admin:task_type.new")
...
class TaskForm(ModelForm):
    class Meta:
        ...
        widgets = {
            "type": QuickAddTaskTypeSelect(editable_model="E-Commerce_tasks.TaskType")
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["assigned_to"].widget.editable_model = "E-Commerce.Contact"
...
class TaskTypeEditView(CreateOrUpdateView):
    ...
    template_name = "E-Commerce/admin/tasks/task_type_edit.jinja"
...
class TaskEditView(BaseTaskViewMixin, CreateOrUpdateView):
    model = Task
    template_name = "E-Commerce/admin/tasks/task_edit.jinja"
    ...
    def get_toolbar(self):
        ...
        obj = self.get_object()
        delete_url = reverse_lazy("E-Commerce_admin:task.delete", kwargs={"pk": obj.pk}) if obj.pk else None
        ...
        if obj and obj.pk:
            if obj.status == TaskStatus.NEW:
                toolbar.append(
                    PostActionButton(
                        post_url=reverse_lazy("E-Commerce_admin:task.set_status", kwargs=dict(pk=obj.pk)),
                        icon="fa fa-check",
                        name="status",
...
class TaskDeleteView(BaseTaskViewMixin, DeleteView):
    model = Task
    success_url = reverse_lazy("E-Commerce_admin:task.list")
...
class TaskSetStatusView(BaseTaskViewMixin, BaseDetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        obj = self.get_object()
        redirect_url = reverse_lazy("E-Commerce_admin:task.edit", kwargs=dict(pk=obj.pk))
