...
from E-Commerce.core.models import AnonymousContact, get_person_contact
from E-Commerce.tasks.admin_module import TaskAdminModule
from E-Commerce.tasks.models import (
    Task, TaskComment, TaskCommentVisibility, TaskStatus, TaskType
)
from E-Commerce.tasks.utils import create_task
...
def test_task_admin(admin_user):
    ...
    # Create new task
    new_task_url = reverse("E-Commerce_admin:task.new")
    ...
    # List Tasks
    list_task_url = reverse("E-Commerce_admin:task.list")
    list_data = {
        "jq": json.dumps({
            "sort": None,
            "perPage": 20,
    ...
    # Add task comment
    edit_task_url = reverse("E-Commerce_admin:task.edit", kwargs=dict(pk=task.pk))
    ...
    set_task_status_url = reverse("E-Commerce_admin:task.set_status", kwargs=dict(pk=task.pk))
    ...
    # Delete task
    delete_url = reverse("E-Commerce_admin:task.delete", kwargs=dict(pk=task.pk))
...
def test_task_type_admin(admin_user):
    ...
    new_task_type_url = reverse("E-Commerce_admin:task_type.new")
    ...
    # List task types
    list_task_type_url = reverse("E-Commerce_admin:task_type.list")
    list_data = {
        "jq": json.dumps({
            "sort": None,
            "perPage": 20,
    ...
    # Edit task type
    edit_task_type_url = reverse("E-Commerce_admin:task_type.edit", kwargs=dict(pk=task_type.pk))
