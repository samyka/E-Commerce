...
from E-Commerce.front.utils.translation import (
...
def test_shop_available_languages(admin_user):
    ...
    with override_settings(
        ...
        client.get("E-Commerce_admin:index")
        assert get_language() == "it"

        # test againt front again
        client.get(reverse("E-Commerce:index"))
        assert get_language() == "fi"
...
def test_admin_set_shop_language(admin_user):
    ...
    with override_settings(
        ...
        edit_url = reverse("E-Commerce_admin:shop.edit", kwargs=dict(pk=shop.id))
        payload = {
