...
from E-Commerce.core.pricing import get_pricing_module, TaxfulPrice, TaxlessPrice
...
def test_known_extra_data():
    ...
    with override_settings(
        E-Commerce_ORDER_KNOWN_SHIPPING_DATA_KEYS=[("instruction", "Instruction")],
        E-Commerce_ORDER_KNOWN_PAYMENT_DATA_KEYS=[("ssn", "Social Security Number")],
        E-Commerce_ORDER_KNOWN_EXTRA_DATA_KEYS=[("wrapping_color", "Wrapping Color")],
    ):
...
def test_anon_disabling():
    with override_settings(E-Commerce_ALLOW_ANONYMOUS_ORDERS=False):
        with pytest.raises(ValidationError):
            order = create_empty_order()
            order.save()
