...
def get_create_payment_url(order_pk):
    return "/api/E-Commerce/order/%s/create_payment/" % order_pk


def get_set_fully_paid_url(order_pk):
    return "/api/E-Commerce/order/%s/set_fully_paid/" % order_pk


def get_order_url(order_pk):
    return "/api/E-Commerce/order/%s/" % order_pk
...
def test_set_fully_paid(admin_user):
    ...
    response = client.post(
        "/api/E-Commerce/order/%s/set_fully_paid/" % order_pk,
        data,
        format="json"
    )
    assert response.status_code == status.HTTP_200_OK
