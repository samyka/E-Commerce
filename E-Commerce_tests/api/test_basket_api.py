...
from E-Commerce.core.pricing import TaxfulPrice
...
from E-Commerce_tests.campaigns.test_discount_codes import (
    Coupon, get_default_campaign
)

from E-Commerce.testing.basket_helpers import (
...
def test_create_new_basket(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        # invalid shop
        response = client.post("/api/E-Commerce/basket/new/", {"shop": 1000})
...
def test_fetch_basket(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        assert basket.shop == shop
        client = get_client(admin_user)
        response = client.get('/api/E-Commerce/basket/{}-{}/'.format(shop.pk, basket.key))
        assert response.status_code == status.HTTP_200_OK
        basket_data = json.loads(response.content.decode("utf-8"))
        assert basket_data["key"] == basket.key
        assert not basket_data["validation_errors"]
        # malformed uuid
        response = client.get('/api/E-Commerce/basket/{}/'.format(basket.key))
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        # wrong shop
        response = client.get('/api/E-Commerce/basket/{}-{}/'.format(shop2.pk, basket.key))
...
def test_add_product_to_basket_with_summary(admin_user):
    ...
    with override_settings(**REQUIRED_SETTINGS):
        ...
        # Get the tax summary
        response = client.get('/api/E-Commerce/basket/{}-{}/taxes/'.format(shop.pk, basket.key))
        assert response.status_code == status.HTTP_200_OK
        response_data = json.loads(response.content.decode("utf-8"))
...
def test_product_is_required_when_adding(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/add/'.format(shop.id, basket.key), payload)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_quantity_has_to_be_in_stock(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        set_configuration()
        from E-Commerce_tests.simple_supplier.utils import get_simple_supplier
        shop = factories.get_default_shop()
        basket = factories.get_basket()
...
def test_update_line_quantity(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/update_quantity/'.format(shop.pk, basket.key), payload)
        assert response.status_code == status.HTTP_200_OK
...
def test_basket_can_be_cleared(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/clear/'.format(shop.id, basket.key), payload)
        response_data = json.loads(response.content.decode("utf-8"))
        assert response.status_code == status.HTTP_200_OK
        basket.refresh_from_db()
...
def test_can_delete_line(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/remove/'.format(shop.id, basket.key), payload)
        response_data = json.loads(response.content.decode("utf-8"))
        assert response.status_code == status.HTTP_200_OK
        basket.refresh_from_db()
        assert basket.product_count == 0
...
def test_add_blank_code(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/add_code/'.format(shop.pk, basket.key), payload)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        response_data = json.loads(response.content.decode("utf-8"))
...
def test_cant_remove_a_dummy_campaign_code(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/remove_code/'.format(shop.pk, basket.key), payload)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_cant_clear_dummy_campaign_codes(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        factories.get_default_shop_product()
        client = get_client(admin_user)
        response = client.post('/api/E-Commerce/basket/{}-{}/clear_codes/'.format(shop.pk, basket.key))
        assert response.status_code == status.HTTP_200_OK
...
def test_can_clear_valid_campaign_codes(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        # clear all codes
        response = client.post('/api/E-Commerce/basket/{}-{}/clear_codes/'.format(shop.pk, basket.key), payload)
        basket_data = json.loads(response.content.decode("utf-8"))
        assert response.status_code == status.HTTP_200_OK
        assert len(basket_data['codes']) == 0
...
def test_set_shipping_address(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/set_shipping_address/'.format(shop.pk, basket.key), payload)
        assert response.status_code == status.HTTP_200_OK
        response_data = json.loads(response.content.decode("utf-8"))
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/set_shipping_address/'.format(shop.pk, basket.key), address_data)
        assert response.status_code == status.HTTP_200_OK
        response_data = json.loads(response.content.decode("utf-8"))
        shipping_addr = response_data["shipping_address"]
...
def test_abandoned_baskets(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/add/'.format(shop.pk, basket1.key), payload)
        assert response.status_code == status.HTTP_200_OK
        response_data = json.loads(response.content.decode("utf-8"))
        assert len(response_data["items"]) == 1
        assert response_data["items"][0]["shop_product"] == shop_product.pk
        ...
        response = client.get("/api/E-Commerce/basket/abandoned/", format="json", data={
            "shop": shop.pk,
            "days_ago": 0,
        ...
        response = client.post('/api/E-Commerce/basket/{}-{}/add/'.format(shop.pk, basket2.key), payload)
        assert response.status_code == status.HTTP_200_OK
        response_data = json.loads(response.content.decode("utf-8"))
        assert len(response_data["items"]) == 1
        assert response_data["items"][0]["shop_product"] == shop_product.pk
...
def test_permissions(admin_user):
    ...
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.get("/api/E-Commerce/basket/{}-{}/".format(shop_one.pk, basket.key))
        assert response.status_code == status.HTTP_200_OK

        # someone figured out the first param is shop!! oh noes
        client = get_client(user_one)
        response = client.get("/api/E-Commerce/basket/{}-{}/".format(shop_two.pk, basket.key))
...
def test_basket_with_methods(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        with override_settings(E-Commerce_ENABLE_MULTIPLE_SHOPS=False):
            ...
            response = client.post('/api/E-Commerce/basket/{}-{}/set_payment_method/'.format(shop.pk, basket.key), data)
            assert response.status_code == status.HTTP_200_OK
            response_data = json.loads(response.content.decode("utf-8"))
            ...
            response = client.post('/api/E-Commerce/basket/{}-{}/set_shipping_method/'.format(shop.pk, basket.key), data)
            assert response.status_code == status.HTTP_200_OK
            response_data = json.loads(response.content.decode("utf-8"))
            ...
            response = client.post('/api/E-Commerce/basket/{}-{}/set_payment_method/'.format(shop.pk, basket.key), {})
            assert response.status_code == status.HTTP_200_OK
            response_data = json.loads(response.content.decode("utf-8"))
            ...
            response = client.post('/api/E-Commerce/basket/{}-{}/set_shipping_method/'.format(shop.pk, basket.key), {})
            assert response.status_code == status.HTTP_200_OK
            response_data = json.loads(response.content.decode("utf-8"))
...
def test_basket_taxes(admin_user, prices_include_tax, tax_rate, product_price, expected_taxful_price, expected_taxless_price):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post("/api/E-Commerce/basket/new/", {"shop": shop.pk}, format="json")
        assert response.status_code == status.HTTP_201_CREATED
        basket_key = response.data['uuid']

        response = client.post('/api/E-Commerce/basket/%s/add/' % basket_key, {'shop_product': shop_product.id}, format="json")
        assert response.status_code == status.HTTP_200_OK
...
def test_create_with_customer(admin_user, user_mode):
    ...
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post("/api/E-Commerce/basket/new/", {"shop": shop.pk, "customer": customer.id}, format="json")
        if user_mode == "normal":
...
def test_set_customer(admin_user, user_mode):
    ...
    with override_settings(**REQUIRED_SETTINGS):
        ...
        # change the customer
        response = client.post('/api/E-Commerce/basket/%s/set_customer/' % uuid, format="json", data={
...
def test_set_customer_group_prices(admin_user):
    ...
    with override_settings(**REQUIRED_SETTINGS):
        ...
        from E-Commerce.customer_group_pricing.models import CgpPrice
        ...
        from E-Commerce.core.pricing._context import PricingContext
        ...
        # add the product
        response = client.post('/api/E-Commerce/basket/%s/add/' % uuid, format="json", data={
            "shop": shop.id,
            "product": product.id
        })
        assert response.status_code == status.HTTP_200_OK
...
def test_set_customer_campaigns(admin_user):
    ...
    with override_settings(**REQUIRED_SETTINGS):
        ...
        from E-Commerce.campaigns.models.basket_line_effects import DiscountFromProduct
        from E-Commerce.campaigns.models.basket_conditions import ContactGroupBasketCondition
...
def test_anonymous(admin_user, with_admin):
    ...
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post("/api/E-Commerce/basket/new/", payload, format="json")
        assert response.status_code == status.HTTP_201_CREATED
        uuid = response.data["uuid"]
        assert response.data["customer"] is None
        ...
        # create the order
        response = client.post('/api/E-Commerce/basket/%s/create_order/' % uuid, format="json")
        assert response.status_code == status.HTTP_201_CREATED
