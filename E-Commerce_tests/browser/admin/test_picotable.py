...
def _visit_list_view(browser, live_server, list_view_name, creator):
    url = reverse("E-Commerce_admin:%s.list" % list_view_name)
...
def _set_settings(browser, setting_type, creator):
    ...
    browser.find_by_css(".E-Commerce-toolbar .btn.btn-inverse").first.click()
    ...
    # save settings
    move_to_element(browser, ".E-Commerce-toolbar .btn.btn-success")
    browser.find_by_css(".E-Commerce-toolbar .btn.btn-success").first.click()
    _check_picotable_item_info(browser, creator)
