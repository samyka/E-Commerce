...
from E-Commerce.core.models import Currency, MutableAddress, Shop, ShopStatus
...
def test_shop_api(admin_user, currency, currency_decimals):
    ...
    response = client.post("/api/E-Commerce/shop/",
    ...
    shop_data["domain"] = "cloud.E-Commerce.com"
    response = client.put("/api/E-Commerce/shop/%d/" % shop.id,
                          content_type="application/json",
                          data=json.dumps(shop_data))
    ...
    response = client.get("/api/E-Commerce/shop/%d/" % shop.id)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get("/api/E-Commerce/shop/")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert shop.domain == data[1]["domain"]

    response = client.delete("/api/E-Commerce/shop/%d/" % shop.id)
    ...
    response = client.delete("/api/E-Commerce/shop/%d/" % default_shop.id)
...
def test_shop_distance(admin_user):
    ...
    response = client.get("/api/E-Commerce/shop/?lat={0}&lng={1}&distance={2}".format(my_position[0], my_position[1], 5),
                          content_type="application/json")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert len(data) == 1
    ...
    response = client.get("/api/E-Commerce/shop/?lat={0}&lng={1}&distance={2}&ordering=-distance".format(my_position[0], my_position[1], 12),
                          content_type="application/json")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert len(data) == 3
    ...
    response = client.get("/api/E-Commerce/shop/?lat={0}&lng={1}&distance={2}".format(my_position[0], my_position[1], 1),
                          content_type="application/json")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert len(data) == 1
