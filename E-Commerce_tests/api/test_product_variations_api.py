...
from E-Commerce.core.models._product_variation import get_all_available_combinations
from E-Commerce.testing.factories import create_product, get_default_shop
...
def test_product_simple_variation(admin_user):
    ...
    response = client.post("/api/E-Commerce/product/%d/simple_variation/" % product1.pk,
    ...
    response = client.delete("/api/E-Commerce/product/%d/simple_variation/" % product1.pk,
                             content_type="application/json",
                             data=json.dumps(variation_data))
...
def test_product_variable_variation_api(admin_user):
    ...
    response = client.post("/api/E-Commerce/product_variation_variable/",
                           content_type="application/json",
                           data=json.dumps(variation_data))
    assert response.status_code == status.HTTP_201_CREATED
    ...
    # fetch
    response = client.get("/api/E-Commerce/product_variation_variable/%d/" % ProductVariationVariable.objects.first().pk)
    ...
    response = client.put("/api/E-Commerce/product_variation_variable/%d/" % ProductVariationVariable.objects.first().pk,
                          content_type="application/json",
                          data=json.dumps(variation_data))
    assert response.status_code == status.HTTP_200_OK
    product.refresh_from_db()
    ...
    # list
    response = client.get("/api/E-Commerce/product_variation_variable/")
    assert response.status_code == status.HTTP_200_OK
    variable_data = json.loads(response.content.decode("utf-8"))
    assert variable_data[0]["translations"]["en"]["name"] == variation_data["translations"]["en"]["name"]

     # delete
    response = client.delete("/api/E-Commerce/product_variation_variable/%d/" % ProductVariationVariable.objects.first().pk)
    assert response.status_code == status.HTTP_204_NO_CONTENT
...
def test_product_variable_variation_value_api(admin_user):
    ...
    # create
    response = client.post("/api/E-Commerce/product_variation_variable_value/",
                           content_type="application/json",
    ...
    # fetch
    response = client.get("/api/E-Commerce/product_variation_variable_value/%d/" % value1.pk)
    assert response.status_code == status.HTTP_200_OK
    variable_data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.put("/api/E-Commerce/product_variation_variable_value/%d/" % value2.pk,
                          content_type="application/json",
                          data=json.dumps(value_data2))
    assert response.status_code == status.HTTP_200_OK

    # list
    response = client.get("/api/E-Commerce/product_variation_variable_value/")
    assert response.status_code == status.HTTP_200_OK
    variable_data = json.loads(response.content.decode("utf-8"))
    ...
    assert variable_data[1]["translations"]["en"]["value"] == value_data2["translations"]["en"]["value"]
     ...
     # delete
    response = client.delete("/api/E-Commerce/product_variation_variable_value/%d/" % value2.pk)
    assert response.status_code == status.HTTP_204_NO_CONTENT
...
def test_product_variable_variation_link(admin_user):
    ...
    response = client.post("/api/E-Commerce/product/%d/variable_variation/" % product1.pk,
                           content_type="application/json",
    ...
    response = client.put("/api/E-Commerce/product/%d/variable_variation/" % product1.pk,
                          content_type="application/json",
                          data=json.dumps(linkage_data))
    assert response.status_code == status.HTTP_200_OK
    result2 = ProductVariationResult.objects.get(combination_hash=hash2)
    ...
    # delete
    response = client.delete("/api/E-Commerce/product/%d/variable_variation/" % product1.pk,
                             content_type="application/json",
