...
((($) => {
    ...
    function getHelpButton(tour, page) {
        let content = "";
        if (page) {
            const helpUrl = window.E-CommerceAdminConfig.docsPage + page;
            content += "<br>";
            content += "<p class='text-center'>";
            content += "<a href='" + helpUrl + "' class='btn btn-inverse btn-default', target='_blank'>";
            content += "<i class='fa fa-info-circle'></i> " + gettext("Learn more at the E-Commerce Help Center");
            content += "</a>";
            content += "</p>";
        }
        return content;
    ...
    function initalizeAndRunTour(config) {
        ...
        if (!config.forceRun && config.tourKey && config.url && window.E-CommerceAdminConfig.csrf) {
            tour.on("cancel", () => {
                $.post(
                    config.url,
                    {
                        "csrfmiddlewaretoken": window.E-CommerceAdminConfig.csrf,
